...
fn catalog_hides_children_of_a_tombstoned_project() {
    ...
    conn.execute_batch(
        "INSERT INTO networks(id,name,schema_version,revision,created_at) VALUES('n','n',2,1,0);
         INSERT INTO devices(id,network_id,display_name,hostname,platform,enrollment_id,revision,created_at) VALUES('live','n','Live','live','test','local-device',1,0);
         INSERT INTO devices(id,network_id,display_name,hostname,platform,enrollment_id,revision,created_at,tombstoned_at) VALUES('retired','n','Retired','retired','test','old-device',1,0,1);
    ...
    assert_eq!(catalog["devices"].as_array().unwrap().len(), 1);
    assert_eq!(catalog["devices"][0]["id"], "live");
    let _ = fs::remove_dir_all(root);
}
