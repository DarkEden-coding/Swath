...
/** Stores the password in a user-only temporary directory and returns its askpass executable. */
function createAskpass(password: string): { dir: string; helper: string } {
...
/** Makes every sudo invocation use Swath's askpass helper, including commands written with `-n`. */
...
export default function sudoPrompt(pi: ExtensionAPI): void {
  let credentials: { dir: string; helper: string } | undefined;
