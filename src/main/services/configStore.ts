...
function pruneMissingWorkspaces(config: AppConfig): AppConfig {
  ...
  return {
    ...
    workspaces,
    activeWorkspaceId:
      config.activeWorkspaceId && workspaces.some((item) => item.id === config.activeWorkspaceId)
  ...
  try {
    ...
    const merged = {
      ...
      settings: {
        ...defaults.settings,
        ...parsed.settings,
        shellProfiles: Array.isArray((parsed.settings as { shellProfiles?: unknown } | undefined)?.shellProfiles)
          ? parsed.settings.shellProfiles
          : defaults.settings.shellProfiles,
        globalEnv:
          parsed.settings?.globalEnv && typeof parsed.settings.globalEnv === "object"
