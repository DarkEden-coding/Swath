...
const GitManagerPane = lazy(() =>
  import("./GitManagerPane").then((module) => ({ default: module.GitManagerPane })),
);

export function createGitManagerPaneMeta(
...
export function createGitManagerView(
  title = "Source Control",
  cwd?: string,
  settings?: AppSettings,
): WorkspaceView {
  const pane = createPaneNode(
    undefined,
    settings ? createGitManagerPaneMeta(settings, cwd) : { kind: "gitManager" },
  );
  return {
    id: createId("view"),
    type: "workspace-view",
