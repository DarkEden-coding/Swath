...
import { IconNetwork, IconSettings } from "../icons";
    ...
    <>
      ...
      >
        ...
        <div className="flex min-w-0 items-center gap-2">
          ...
          <button
            type="button"
            className="grid size-[30px] shrink-0 cursor-pointer place-items-center rounded-md border-0 bg-transparent text-swath-muted-2 [-webkit-app-region:no-drag] [app-region:no-drag] hover:bg-swath-panel-2 hover:text-swath-accent-strong"
            title="Network map"
            aria-label="Open network map"
            onClick={() => setNetworkMapOpen(true)}
          >
            <IconNetwork width={17} height={17} strokeWidth={1.35} className="block" />
          </button>
          <button
            type="button"
      ...
      <NetworkMapModal open={networkMapOpen} onClose={() => setNetworkMapOpen(false)} />
    </>
  );
}
